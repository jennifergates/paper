# *****************************************************************************
# WebSockets Message Not Normal Bro script
# Jennifer Gates
# August 2017
# 
# Module to parse and log WebSocket messages that are not normal activity  
# the custom apps on the Damn Vulnerable Web Sockets web server.  Logs uid, 
# client IP, server IP, server port, opcode, mask key, uri, and data to 
# WS_MessageNotNormal.log and creates entries in the Bro notice.log with more
# specific descriptions of not normal behavior.
# *****************************************************************************
...
# Create some constants for the DVWS URIs and 
# Regular Expressions to match expected responses or known bad entries
...
event ws_unmaskedmessage(c: connection, first2B: Brofirst2B, data: string) {
	...
	# if normal, the response only includes letters and possibly a single quote
