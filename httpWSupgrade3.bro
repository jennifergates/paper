...
#use http_all_headers event as defined in Bro_HTTP.events.bif.bro
event http_all_headers(c: connection, is_orig: bool, hlist: mime_header_list)
{
	...
	for (i in hlist)
	{
		print "New header -----------------------------------------";
		print hlist[i];
		# if this is a server response to a successful handshake 
		if ("UPGRADE" in hlist[i]$name && "websocket" in hlist[i]$value && c$http?$status_code && c$http$status_code == 101)  #the ? lets you check if it exists
		{
			...
			for (x in hlist)
			{
				...
				{
					local acceptkey=hlist[x]$value;
				};

				if ("WEBSOCKET-ORIGIN" in hlist[x]$name)
				{
					local svrorigin=hlist[x]$value;
				};

				if ("WEBSOCKET-LOCATION" in hlist[x]$name)
				{
					local svrlocation=hlist[x]$value;
				};
		...
		# if this is a client request to handshake 
		if ("SEC-WEBSOCKET-VERSION" in hlist[i]$name && "13" in hlist[i]$value )  
		{
			...
			for (y in hlist)
			{
				...
				{
					local key=hlist[y]$value;
				} ;

				if ( "ORIGIN" in hlist[y]$name )
				{
					local origin=hlist[y]$value;
				} ;
